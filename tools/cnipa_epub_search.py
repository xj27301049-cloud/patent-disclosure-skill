...
浏览器后端（二选一）：
- Node.js：在 tools/ 执行 npm install（推荐，也作为自动降级后端）；
- Python：pip install -r tools/requirements-cnipa.txt && python -m playwright install chromium。

脚本优先使用 Python Playwright；缺失时自动切换到 Node/Puppeteer + Chromium。
...
环境变量：公共项含 ``EPUB_WAF_MAX_WAIT_SEC``；Python 后端另见 ``cnipa_epub_crawler.py``，
Node 后端可用 ``PATENT_BROWSER_RUNTIME_DIR`` 指定浏览器运行时缓存目录。
"""
...
def _search_epub_keyword_node(keyword: str):
    """Use the bundled Node/Puppeteer backend when Python Playwright is unavailable."""
    from cnipa_epub_parse import parse_search_result_html

    node = os.environ.get("CODEX_PRIMARY_RUNTIME_NODE") or shutil.which("node")
    runner = Path(__file__).resolve().with_name("cnipa_epub_node.mjs")
    if not node or not runner.exists():
        raise RuntimeError(
            "No browser backend: Python Playwright is missing and the Node fallback is unavailable"
        )
    timeout = max(240, int(float(os.environ.get("EPUB_WAF_MAX_WAIT_SEC", "180"))) + 150)
    proc = subprocess.run(
        [node, str(runner), keyword],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
    ...
    if proc.returncode != 0:
        detail = (proc.stderr or proc.stdout or "unknown Node backend error").strip()
        network_markers = (
            "ERR_EMPTY_RESPONSE",
            "ERR_CONNECTION",
            "ERR_TIMED_OUT",
        ...
        if any(marker in detail for marker in network_markers):
            raise RuntimeError(
                "Official CNIPA epub endpoint is currently unavailable from this runtime; "
                "use the verified WebSearch/Google Patents fallback required by prior_art_search.md"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        search_epub_keyword = _search_epub_keyword_node
