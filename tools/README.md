...
浏览器后端优先级：Python Playwright + Chromium；若缺失，`cnipa_epub_search.py` 会自动改用随工具安装的 Node/Puppeteer + Chromium 后端。可用 `node tools/cnipa_epub_node.mjs --smoke` 做浏览器内核自检。环境变量见各脚本文件头。默认结果 HTML 落在 **`tools/_last_result_*.html`**（已 `.gitignore`）。
